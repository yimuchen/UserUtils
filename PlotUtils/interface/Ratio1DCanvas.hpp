...
public:
  ...
  inline Top1DPad&TopPad(){ return GetPad<Top1DPad>( 0 ); }

  /** @brief returning reference to bottom pad object */
  inline Bottom1DPad&BottomPad(){ return GetPad<Bottom1DPad>( 1 ); }
  ...
  inline TH1D&
  PlotScale( const TH1D& x, const TH1D& y )
  {
    return PlotScale( x, y, {} );
  }
  inline TH1D&PlotScale( const TH1D*                   x,
                         const TH1D*                   y,
                         const std::vector<RooCmdArg>& l )
  ...
  inline TGraphAsymmErrors&PlotScale( const TGraph*                 x,
                                      const TGraph*                 y,
                                      const std::vector<RooCmdArg>& l )
  { return PlotScale( *x, *y, l ); }
  inline TGraphAsymmErrors&
  ...
  inline TGraphAsymmErrors&PlotPull( const TGraph*                 x,
                                     const TGraph*                 y,
                                     const std::vector<RooCmdArg>& l )
  ...
  template<typename ... Args>
  inline TGraphAsymmErrors&PlotPull( const TGraph& x, const TGraph&    y,
