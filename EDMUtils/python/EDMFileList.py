...
def ListEDMDatasets(primary_tag, process_tag, tier):
  ...
  query_proc = subprocess.Popen(shlex.split(query_cmd),
                                stdout=subprocess.PIPE,
                                stderr=subprocess.PIPE)
  grep_proc = subprocess.Popen(shlex.split(grep_cmd),
...
def ListFileInDataset(dataset):
  query_cmd = 'das_client --limit=10000000 --query="file dataset={}"'.format(
      dataset)
  grep_cmd = 'grep "/store"'
